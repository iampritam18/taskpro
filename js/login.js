      ...
      fetch("http://localhost:5001/api/auth/login", {
        ...
        .then((data) => {
          ...
          if (data.token) {
            ...
            document.querySelector("#loginCard .auth-tabs").style.display =
              "none";
            loginSuccess.querySelector("h3").textContent =
              `Welcome, ${data.name}! 🎉`;
    ...
    if (valid) {
      ...
      fetch("http://localhost:5000/api/auth/register", {
        ...
        .catch((err) => {
          ...
          btn.classList.remove("loading");
          btn.textContent = "Create Account";
          showToast(`Server error: ${err.message || "Is backend running?"}`);
        });
    }
  });
