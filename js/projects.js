...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateStats() {
    ...
    document.getElementById("completedProjectsCount").textContent =
      allProjects.filter((p) => p.progress === 100 && p.totalTasks > 0).length;
  ...
  function openEditModal(project) {
    ...
    if (project.deadline) {
      document.getElementById("projectDeadline").value = new Date(
        project.deadline,
      )
        .toISOString()
        .split("T")[0];
  ...
  function showLoading(isLoading) {
    ...
    if (isLoading) {
      grid.innerHTML =
        '<div class="loading-state"><div class="spinner"></div><p>Syncing workspace...</p></div>';
    }
  }
